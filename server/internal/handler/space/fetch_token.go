...
func FetchSpaceToken(httpCtx *fasthttp.RequestCtx, client *firestore.Client) {
	...
	space, err := models.GetSpace(client, firebaseCtx, fetchRequest.SpaceId)
	if err != nil {
	...
	if space.SpaceToken == nil {
		...
		spaceToken, generateErr = auth.GenerateSpaceToken(firebaseCtx, client, fetchRequest.SpaceId)
		if generateErr != nil {
		...
		valid, err := auth.ValidateSpaceToken(firebaseCtx, client, spaceToken, fetchRequest.SpaceId)
