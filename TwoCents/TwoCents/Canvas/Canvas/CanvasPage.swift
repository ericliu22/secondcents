    ...
    func GridView() -> some View {
        ForEach(viewModel.canvasWidgets, id: \.id) { widget in
            ...
            MediaView(widget: widget, spaceId: spaceId)
                ...
                .position(
                    x: widget.x ?? FRAME_SIZE / 2, y: widget.y ?? FRAME_SIZE / 2
                )
                ...
                .draggable(widget) {
                    MediaView(widget: widget, spaceId: spaceId)
                        .contentShape(
                            .dragPreview,
                            RoundedRectangle(
                                cornerRadius: CORNER_RADIUS, style: .continuous)
                        )
                        .frame(
                            width: widget.width,
                            height: widget.height
    ...
    var body: some View {
        ...
        ZoomableScrollView {
            canvasView()
                ...
                .navigationTitle(
                    viewModel.isDrawing ? "" : viewModel.space?.name ?? ""
                )
                ...
                .onAppear(perform: {
                    ...
                    Task {
                        await readNotifications(
                            spaceId: spaceId, userId: appModel.user!.userId)
                    }
                })
                .task {
                    do {
                    ...
                    if let userInSpace = try? await viewModel.space?.members?
                        .contains(getUID() ?? "")
                    {
                        print(userInSpace)
                        if !userInSpace {
